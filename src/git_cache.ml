...
module Raw = struct
  module Commands = struct
    ...
    module Key = struct
      ...
      let to_json { commit; remote_origin; commit_message } =
        `Assoc
          [
    ...
    let build { path } (job : Current.Job.t) (k : Key.t) :
      ...
      let { commit = _; remote_origin; commit_message } : Key.t = k in
...
let add_commit_push ?schedule ~label ~remote_origin ~commit_message src d =
  ...
  let k = Raw.Commands.Key.{ commit; remote_origin; commit_message } in
